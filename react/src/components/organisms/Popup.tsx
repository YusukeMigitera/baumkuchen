...
export const Popup: FC<Props> = memo((props) => {
  const { togglePopup, onClickToggle, sendUser } = props
        ...
        <div className="relative p-4 w-full max-w-md h-full md:h-auto">
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <div className="p-6 text-center m-auto">
              <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-white">Would you choose the best answer?</h3>
              <p className='dark:text-white mb-5 text-ms font-normal'>Price {String(priceValue)} ETH</p>
              <button 
              type="button" 
              className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2"
              onClick={() => send(sendUser, String(priceValue))}
              >
                  Yes, I'm sure
              </button>
              <button 
